pipeline {
    ...
    environment {
        ...
        PROJECT_DIR = "D:\\devops\\NEW_PROJECT_FILE\\15projects30days\\project_4\\Subscription_man"
        COMPOSE_FILE = "D:\\devops\\NEW_PROJECT_FILE\\15projects30days\\project_4\\Subscription_man\\docker-compose.yml"
        K8S_DIR = "D:\\devops\\NEW_PROJECT_FILE\\15projects30days\\project_4\\Subscription_man\\k8s"
        
        COMPOSE_PROJECT_NAME = "subscription-man"
        BACKEND_HEALTH_URL = "http://localhost:5000/health"
        DOCKER_HUB_USER = "rajbirari9737"
        DOCKER_HUB_CREDS = "dockerhubcreadentials"
        K8S_CONTEXT     = "docker-desktop"
        NAMESPACE       = "subscription-man"
    }
            ...
            steps {
                dir(env.PROJECT_DIR) {
                    ...
                    script {
                        ...
                        bat 'docker rm -f subscription_backend subscription_frontend subscription_db 2>nul || exit 0'
                    }
                ...
                dir(env.PROJECT_DIR) {
                    ...
                    script {
                        try {
                            ...
                            bat 'kubectl apply -f "%K8S_DIR%" --validate=false'
                            
                            echo 'Verifying deployment status...'
                            bat 'kubectl rollout status deployment/backend -n subscription-man --timeout=90s'
                            bat 'kubectl rollout status deployment/frontend -n subscription-man --timeout=90s'
